...
func parseOperator(args []string) (attemptCommand, bool, bool) {
	...
	case commandAgentIdlePolicy:
		if command.agent == "" || command.expectedRevision == 0 || (command.provider == "wait" && (command.maxRunSeconds != 0 || command.text != "" || command.toolBudget != 0)) || (command.provider == "standing_instruction" && (command.maxRunSeconds == 0 || command.text == "" || command.toolBudget == 0)) {
			return attemptCommand{}, false, false
		}
	case commandTaskAdd:
