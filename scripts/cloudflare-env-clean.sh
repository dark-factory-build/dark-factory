...
if [ "${DARK_FACTORY_CLOUDFLARE_CLEAN_LAUNCH:-}" != reviewed-v1 ]; then
    echo "cloudflare-admin: invoke through scripts/with-cloudflare-env.sh" >&2
    exit 1
fi
unset DARK_FACTORY_CLOUDFLARE_CLEAN_LAUNCH
...
verify_reviewed_tree() {
    ...
    clean_git -C "$repository_root" \
        cat-file -e "$reviewed_commit:scripts/cloudflare-env-clean.sh" || {
        echo "cloudflare-admin: clean-environment helper is not committed at the reviewed commit" >&2
        return 1
    }
    clean_git -C "$repository_root" \
        diff --quiet --no-ext-diff "$reviewed_commit" -- \
        scripts/with-cloudflare-env.sh scripts/cloudflare-env-clean.sh \
        cmd/cloudflare-admin internal/cloudflareadmin || {
