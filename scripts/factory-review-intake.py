...
def gate_failure_note(receipt, operation):
    ...
    try:
        # Only failure lines name a test: Go's "--- FAIL: Name", unittest's
        # "FAIL: name" / "ERROR: name". Matching every "test" word reported
        # "test-renderer" and "testing" from ordinary log text.
        tests = list(dict.fromkeys(re.findall(r"(?m)^(?:--- FAIL|FAIL|ERROR): (\S+)", log.read_text(encoding="utf-8"))))[:16]
    except OSError:
        pass
