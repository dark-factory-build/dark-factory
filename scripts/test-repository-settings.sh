...
assert_job_field relay needs "scope"
assert_job_field relay if "needs.scope.result == 'success' && needs.scope.outputs.relay == 'true'"
...
grep -Eq '^/docs/development/WORKFLOW\.md[[:space:]]+@baziyer([[:space:]]|$)' "$repository_root/.github/CODEOWNERS" || { echo 'missing repository contract: active @baziyer rule for /docs/development/WORKFLOW.md' >&2; exit 1; }
