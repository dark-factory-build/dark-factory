...
prompt="$prompt Review precedes enqueue. Protected combined-tree CI runs after enqueue and must pass before merge; pending post-enqueue checks are a deferred delivery condition, not by themselves a source-review defect. Still block concrete defects and false claims of passing checks."
cd "$work"
case "$provider" in
    codex)
