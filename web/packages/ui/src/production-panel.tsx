...
export const reviewProse = (value: string) => value.replace(/<!--\s*dark-factory-operation:[\s\S]*?-->/gi, "").replace(/\n{3,}/g, "\n\n").trim();
function ReviewNotes({ findings }: { findings: string }) {
  const prose = reviewProse(findings);
  if (!prose) return null;
  return prose.length > 600 ? <details><summary>Review notes</summary><p className="dfRecentWorkText">{prose}</p></details> : <p className="dfRecentWorkText">{prose}</p>;
}
...
export function ProductionPanel({ maintenance, deliveries = [], items, selected, onSelect, state, call, connected = true, error, overflow = 0, loadMore, onMission, onAgent, onLoadTaskDetail, onLoadTaskHistory }: {
  ...
  return <section className="dfConsoleSidebar__panel" aria-label="Production inspection">
    ...
    {active === "delivery" ? <section aria-label="Delivery dock"><h3>Delivery dock</h3>{deliveries.length === 0 ? <p>No project delivery evidence recorded. Runtime release and installation receipts belong to Factory service.</p> : <><p>Shared project release receipts. These timestamps record when each destination was verified.</p><DeliveryEvidence deliveries={deliveries} current={connected && !error} /></>}</section> : null}
    {item === undefined ? null : <article className="dfRecentWorkDetail" aria-label={`Production details for ${item.pullRequest?.title || item.construction?.title || "work"}`}>
      ...
      <p><strong>Next action:</strong> {reviewProse(item.nextAction) || "Review notes recorded."}</p>
      ...
      <h4>Review</h4><p>{item.review.state} · {item.review.current ? "current head" : item.review.head ? "stale head" : "no head recorded"}{item.review.sourceFresh ? "" : " · source observation stale or unavailable"}</p><ReviewNotes findings={item.review.findings} />{href(item.review.url) ? <a href={item.review.url} target="_blank" rel="noreferrer">Open review</a> : null}
      {item.reviewers.length ? <ul>{item.reviewers.map((reviewer) => { const findings = reviewProse(reviewer.findings ?? ""); return <li key={reviewer.id}>{reviewer.name} · {reviewer.provider} · {reviewer.state} · head <code style={{ overflowWrap: "anywhere" }}>{reviewer.head}</code>{findings ? ` · ${findings}` : ""}{href(reviewer.url) ? <> · <a href={reviewer.url} target="_blank" rel="noreferrer">Evidence</a></> : null}</li>; })}</ul> : <p>No reviewer assignment recorded.</p>}
