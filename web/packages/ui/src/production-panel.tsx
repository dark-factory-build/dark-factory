...
export function ProductionPanel({ items, selected, onSelect, state, call, connected = true, error, overflow = 0, loadMore, onMission, onAgent, onLoadTaskDetail, onLoadTaskHistory }: {
  ...
  return <section className="dfConsoleSidebar__panel" aria-label="Production inspection">
    ...
    {items.length === 0 ? <p className="dfFactoryConsole__empty">No production records in this scope.</p> : <label>Inspect work<select value={active ?? ""} onChange={(event) => inspect(event.target.value)}><option value="">Select work</option>{visibleItems.map((entry) => { const key = `${entry.projectId}:${entry.visualId}`; const title = entry.pullRequest?.title || entry.construction?.title || "Work"; return <option key={key} value={key}>{title} · {entry.repository} · {entry.status}</option>; })}</select></label>}
    ...
    {item === undefined ? null : <article className="dfRecentWorkDetail" aria-label={`Production details for ${item.pullRequest?.title || item.construction?.title || "work"}`}>
      ...
      <h4>Merge and delivery</h4><p>{item.pullRequest?.merge ? `Merged at ${item.pullRequest.merge}` : item.pullRequest?.state === "closed" ? "Closed without merge" : "Not merged"}</p>{item.deliveries.length === 0 ? <p>No delivery evidence recorded.</p> : <ul>{item.deliveries.map((delivery) => <li key={`${delivery.repository}:${delivery.id}`}>{delivery.destination} · {delivery.state} · revision <code style={{ overflowWrap: "anywhere" }}>{delivery.revision}</code>{delivery.verified ? " · verified" : " · not verified"}{delivery.overflow ? ` · ${delivery.overflow} additional PR links not loaded` : ""}{delivery.pull_requests.length > 1 ? ` · includes ${delivery.pull_requests.length} PRs` : ""}{href(delivery.url) ? <> · <a href={delivery.url} target="_blank" rel="noreferrer">Open delivery</a></> : null}</li>)}</ul>}
