...
export function MissionsPanel({ production = [], onProduction, requestedMission, state, projectId, active, call, onProject, onSelectAgent, onOpenTask }: {
  ...
  useEffect(() => { generation.current++; setItems([]); setSelected(undefined); setTasks([]); setTaskNext(0); setLoaded(false); setNext(0); setPending(""); setNotice(""); setError(""); }, [projectId]);
  useEffect(() => () => { generation.current++; }, []);
  const run = async (work: () => Promise<void>, background = false, saving = false) => {
    if (pending || call === undefined || projectId === undefined) return;
    const current = generation.current;
    setPending(saving ? "Saving mission…" : "Loading…"); setError(""); if (!background) setNotice("");
  ...
  const read = async (id: string) => {
    ...
    if (current === generation.current) { lastObservedHead.current = state?.head; setSelected(value); await loadTasks(id); }
  };
  ...
  return <section className="dfMissions dfConsoleSidebar__panel" aria-label="Missions">
    ...
    {project === undefined ? <label>Choose a project<select aria-label="Mission project" value="" onChange={(event) => onProject(event.target.value)}><option value="" disabled>Select project</option>{[...state?.projects.values() ?? []].map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label> : <>
      ...
      {draft === undefined ? null : <form className="dfConsoleSidebar__config" onSubmit={(event) => { event.preventDefault(); void run(async () => {
        ...
        <label>Objective<textarea required maxLength={8192} value={draft.objective} disabled={Boolean(pending)} onChange={(event) => changeDraft({ objective: event.target.value })} /></label>
        <label>Acceptance criteria<textarea required maxLength={8192} value={draft.criteria} disabled={Boolean(pending)} onChange={(event) => changeDraft({ criteria: event.target.value })} /></label>
        <label>Overseer<select value={draft.owner} disabled={Boolean(pending)} onChange={(event) => changeDraft({ owner: event.target.value })}>{owners.map((agent) => <option key={agent.id} value={agent.id}>{agent.name}</option>)}</select></label>
      ...
      {selected === undefined ? null : <article className="dfConsoleSidebar__section">
        ...
        <details><summary>Acceptance criteria</summary><p className="dfMissions__text">{text(document.criteria)}</p></details>
        {text(document.reason) || text(document.conclusion) || rows(document.links).length || (Array.isArray(selected.missing_references) && selected.missing_references.length) ? <details><summary>Evidence</summary>
          {text(document.reason) ? <p className="dfMissions__text">{text(document.reason)}</p> : null}
          {text(document.conclusion) ? <p className="dfMissions__text">{text(document.conclusion)}</p> : null}
          {rows(document.links).map((link, index) => <p key={index}>{text(link.relation)}{text(link.stage) ? ` · ${text(link.stage)}` : ""}{/^https:\/\//.test(text(link.source)) ? <> · <a href={text(link.source)} target="_blank" rel="noopener noreferrer">Open evidence</a></> : null}</p>)}
