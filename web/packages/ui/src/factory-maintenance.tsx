...
function Identity({ value, state }: { value: FactoryBuild; state?: string }) {
  return <>
    <p><code>{value.version || "not observed"}</code> · {state || "unknown"}</p>
    <details><summary>Build details</summary><p>{value.release ? "release receipt" : "no release receipt"}</p><p>Source <code style={{ overflowWrap: "anywhere" }}>{value.source || "not observed"}</code></p><p>Target <code>{value.target || "not observed"}</code></p><p>Build <code style={{ overflowWrap: "anywhere" }}>{value.build_id || "not observed"}</code></p></details>
  </>;
}
...
export function FactoryMaintenancePanel({ maintenance, runtime, connected, sourceFresh, hostedSource, deliveries }: FactoryMaintenanceProps) {
  ...
  return <section className="dfMaintenance" aria-label="Factory service maintenance">
    ...
    <section aria-label="Host"><h4>Host</h4><dl>
      <dt>Available release</dt><dd>{maintenance ? <><p><code>{maintenance.available.version || "not observed"}</code> · {maintenance.available.state}</p>{link(maintenance.available.url) ? <a href={maintenance.available.url} target="_blank" rel="noreferrer">Open release</a> : <p>No release link observed.</p>}</> : <p>Not observed.</p>}</dd>
      <dt>Installed service files</dt><dd>{maintenance ? <Identity value={maintenance.installed} state={maintenance.installed.state} /> : <p>Not observed.</p>}</dd>
      <dt>Running host</dt><dd>{running ? <Identity value={running} state={runtime && connected ? "observed through this connection" : "last observed"} /> : <p>Not observed.</p>}</dd>
    </dl></section>
    <section aria-label="Hosted console"><h4>Hosted console</h4><p>{hostedSource ? `Loaded source ${hostedSource.slice(0, 8)}` : "Source not provided."}</p><details><summary>Build details</summary><p>Source <code style={{ overflowWrap: "anywhere" }}>{hostedSource || "not provided"}</code></p><p>The console bundle and host runtime can run different revisions.</p></details></section>
    <details><summary>Delivery evidence</summary><DeliveryEvidence deliveries={deliveries} current={current} /></details>
