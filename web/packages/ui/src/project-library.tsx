...
export function ProjectLibrary({ state, call, draft }: { state?: StateView; call?: ProjectContentCall; draft?: (agent: AgentItem, instruction: string) => void }) {
  ...
  return <details className="dfConsoleSidebar__panel dfProjectLibrary">
    ...
    <p>Optional procedures and scenario definitions. Observed results stay separate; tasks use exact revisions.</p>
    ...
    <fieldset disabled={pending || call === undefined || projectID === ""}>
      ...
      {selected === undefined ? null : <section aria-label="Selected library revision">
        ...
        <button type="button" disabled={selected.deprecated === true || selected.revision !== selected.latest_revision} onClick={() => void run(async () => { const value = await request("deprecate", { id: selected.id, expected_revision: selected.revision }); setSelected(value); setNotice("New deprecated revision saved. Existing attachments are unchanged."); })}>Deprecate</button>
        <button type="button" onClick={() => void run(() => loadEvidence())}>Observed evidence</button>
        ...
        <details><summary>Record observed evidence</summary><form onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); void run(async () => { await request("evidence", { id: data.get("id"), content_id: selected.id, content_revision: selected.revision, tested_source: data.get("tested_source"), environment: data.get("environment"), result: data.get("result"), location: data.get("location"), judgment: data.get("judgment") }); evidenceID.current = id(); setNotice("Observed evidence saved separately from the scenario definition."); }); }}>
          <input type="hidden" name="id" value={evidenceID.current} />
          <label>Tested source <input name="tested_source" required /></label><label>Environment <input name="environment" required /></label><label>Observed result <textarea name="result" required /></label><label>Existing report or result reference <input name="location" required /></label><label>Judgment <textarea name="judgment" /></label><button>Record observation</button>
        </form></details>
        <form onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); void run(async () => { await request("attach", { task_id: data.get("task"), content_id: selected.id, content_revision: selected.revision }); setNotice("Exact revision attached to the task’s current work revision."); }); }}>
