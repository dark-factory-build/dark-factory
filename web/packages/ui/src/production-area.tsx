    ...
    {[{ key: "maintenance", title: "FACTORY SERVICE", receipts: factoryDeliveries }, { key: "delivery", title: "DELIVERY DOCK", receipts: projectDeliveries }].map((dock, index) => {
      ...
      return <g key={dock.key} transform={`translate(${16 + index % columns * cell} ${dockTop + Math.floor(index / columns) * 120})`}>
        <g aria-hidden="true" pointerEvents="none">
          ...
          <text x="8" y="105" fill="#8fa4ac" fontFamily="ui-monospace, monospace" fontSize="9">{latest ? label(`${latest.destination.startsWith("runtime:") ? "Runtime" : latest.destination.startsWith("site:") ? "Console" : "Delivery"} · ${latest.phase || latest.state}`) : "No delivery evidence · inspect"}</text>
        </g>
