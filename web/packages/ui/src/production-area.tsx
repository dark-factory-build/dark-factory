...
export function ProductionArea({ view, items, width, top, pulse, onSelect, selected }: {
  ...
  return <g transform={`translate(0 ${top})`} role="group" aria-label="Production area">
    ...
    {items.map((item, index) => {
      ...
      const failure = checks.some((check) => ["failure", "timed_out", "action_required"].includes(check.conclusion));
      const running = checks.some((check) => check.state === "in_progress");
      ...
      const checkState = failure ? "failed" : running ? "running" : checks.some((check) => check.state === "queued") ? "queued" : checks.length === 0 ? "unknown" : checks.every((check) => check.conclusion === "success") ? "passed" : "inspect";
    ...
    {executions.map((check, index) => {
      ...
      const running = item?.review.sourceFresh && check.state === "in_progress" && pulse !== undefined;
      return <g key={check.id} transform={`translate(${16 + index % columns * cell} ${60 + Math.max(1, Math.ceil(items.length / columns)) * 144 + Math.floor(index / columns) * 104})`} {...activate(() => { if (item) onSelect(item.projectId + ":" + item.visualId); })} aria-label={`Inspect shared check ${check.name}`} className="dfFactoryScene__target">
        ...
        <path d={running && Math.floor(pulse / 400) % 2 ? "M20 26h6l4-8 5 14 5-6h9" : "M20 26h29"} stroke={check.conclusion === "success" ? "#9fe7b0" : check.conclusion === "failure" ? "#d49b7d" : "#8fa4ac"} fill="none" />
