...
export type TopologyView = Readonly<{ projectId: string; digest: string; sourceRevision: string; nodes: readonly TopologyBody["nodes"][number][]; dependencies?: Readonly<{ source: "go-imports-package-manifests"; edges: readonly Readonly<{ from: string; to: string; weight: number }>[]; omitted: number }> }>;
...
export class BrowserSession {
  ...
  #consoleResult(frame: Extract<ServerControlFrame, { type: "AGENT_UPDATE_RESULT" | "PROJECT_LIMITS_RESULT" | "TASK_UPDATE_RESULT" | "TOPOLOGY" | "RUN_PATHS" | "TASK_LIST" }>): void {
    ...
    pending.resolve(Object.freeze({ projectId: frame.body.project_id, digest: frame.body.digest, sourceRevision: frame.body.source_revision, nodes: Object.freeze(frame.body.nodes.map((node) => Object.freeze({ ...node }))), ...(frame.body.dependencies === undefined ? {} : { dependencies: Object.freeze({ ...frame.body.dependencies, edges: Object.freeze(frame.body.dependencies.edges.map((edge) => Object.freeze({ ...edge }))) }) }) }) as never);
  }
