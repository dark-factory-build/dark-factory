...
9. Contributor agents use only the repository-bound Maintainer App for GitHub.
   ...
   unavailable. Never substitute authenticated `git`/`gh`, PATs, SSH agents,
   credential helpers, browser sessions, keychains, or provider GitHub tools.
   Human operators may perform a separately reviewed GitHub action normally.
