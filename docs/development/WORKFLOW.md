...
binaries, writes its receipt and launchd plist, and loads that job; over an
installation this home already proves is its own, a different build uninstalls
and installs again in the one call. It does not download a release, update a
different installation, migrate another home, maintain version pointers, or
promise rollback. See
