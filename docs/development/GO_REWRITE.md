...
| In progress but held | Daemon-side terminal lease/input/resize and reserved HumanRequest delivery now have reviewed Store, runner and owner-mailbox foundations but not the exact correlated effect choreography. The browser-v1 Go/TypeScript state union is repairing adversarially discovered reducer and strict-wire defects before it can land. Recovery still needs replay onto the PTY design. The development/Go sub-gates are integrated and green; final browser E2E and the post-test system census remain cutover blockers. |
...
| Blocked until proved | Exact daemon delivery of lease generation/input/resize and a reserved HumanRequest reply; loopback transport authentication/Host/Origin security over the durable browser authority; repaired browser protocol state operations and TypeScript connection/reconnect client; public web UI; complete private host integration; revised crash-cut vertical slice |
