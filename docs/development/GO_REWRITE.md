...
The next critical-path slices are intentionally held outside canonical while
they are proved: exact runner-start/AttemptResult Store grammar in
`.worktrees/attempt-result-kernel`, and one joined global scheduler at
`f2f95d0f`. The scheduler is not wired into `factoryd`; a finite synchronous
startup recovery pass must land first. In the private site repository, real
hosted-origin pairing/state/refresh against canonical runtime `45dcfee5` is
repaired at `732275a` and remains held for fresh independent review. That
harness is bootstrap evidence, not yet the full provider terminal and
HumanRequest product loop.
...
| In progress but held | Exact runner-start/AttemptResult Store grammar; the unactivated single-owner scheduler at `f2f95d0f`; finite startup recovery and final live result publication; private-site real-loop repair `732275a` pending fresh review. The development/Go sub-gates are integrated and green. |
