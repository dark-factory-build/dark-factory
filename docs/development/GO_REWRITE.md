...
- Reality corrected three reviewed launchctl assumptions that fakes had
  ...
  census, and every durable member remain exact. Service-root directory
  identity intentionally excludes ctime because legitimate entry churn
  updates it; a transient rename-away/decoy/rename-back after the
  recheck remains a residual blind window, while in-window identity and
  binding checks still fail closed.
