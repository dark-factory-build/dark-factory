...
| In progress but held | Provider descriptor candidate `7f3c3c77` is focused normal/race green and under exact review before canonical replay. Release and recovery still need replay onto the final PTY/provider design. The development/Go sub-gates are integrated and green. |
...
| Blocked until proved | Production provider integration; independent review and integration of the real Go-server TypeScript terminal/HumanRequest proof; interactive terminal/reply/cancel expansion of the public UI; exact private-host product-loop proof; remaining factoryctl recovery/cutover plumbing; revised crash-cut vertical slice |
