...
Global Store-owned admission is canonical through `f7c0af18`; its integration
compile repair and different-agent last-capacity causal test are being
re-reviewed on the current head. Read-only launchd service status is canonical
through `960f55d5` and received independent exact-head **ALLOW** after three
rounds of fail-closed race attacks. Provider candidate `7f3c3c77` replaces the
Shell wrapper/control-frame startup path with one unlinked read-only task
descriptor and is under fresh exact review before replay onto canonical head.
...
historical focused-gate evidence only and must be replayed after the integrated
provider/runtime graph is fixed.
...
| In progress but held | Global admission is integrated and its final different-agent capacity proof is under exact re-review. Provider descriptor candidate `7f3c3c77` is focused normal/race green and under exact review before canonical replay. Release and recovery still need replay onto the final PTY/provider design. The development/Go sub-gates are integrated and green. |
...
| Blocked until proved | Final global-admission re-review; production provider integration; independent review and integration of the real Go-server TypeScript terminal/HumanRequest proof; interactive terminal/reply/cancel expansion of the public UI; exact private-host product-loop proof; remaining factoryctl recovery/cutover plumbing; revised crash-cut vertical slice |
