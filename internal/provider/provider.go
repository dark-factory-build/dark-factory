...
func Build(request Request) (Launch, error) {
	...
	case kernel.ProviderCodex:
		...
		if request.role == kernel.RoleOrchestrator {
			prompt += " Publication rule: for a GitHub issue-sourced Change that completes the source issue, set close_on_merge true so the App-rendered PR footer is Closes #N; when the PR only advances an umbrella issue, set it false so the footer is Refs #N. Do not invent a footer without an exact source issue."
		}
