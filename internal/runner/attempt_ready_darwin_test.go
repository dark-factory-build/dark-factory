...
func TestAttemptControllerNextReadyDoesNotConsumePartialFrame(t *testing.T) {
	...
	if err := controller.Configure(AttemptSpec{AttemptID: "readiness", Wrapper: wrapper, MarkerName: InnerActivationMarkerName, ResultName: AttemptResultSpoolName, ResultProof: testResultProof()}); err != nil {
		t.Fatal(err)
	}
