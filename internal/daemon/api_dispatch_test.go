...
func TestDaemonSelectOverseerAccountPreservesSelectionGuards(t *testing.T) {
	...
	ctx := context.Background()
	projectID := testID(60)
	workerID, overseerID, archivedID := testID(61), testID(62), testID(63)
	accountID := testID(64)

	done := fixture.serve(t)
	if _, err := client.CreateProject(ctx, api.CreateProjectInput{ID: projectID, Name: "project", Root: filepath.Join(t.TempDir(), "source")}); err != nil {
		t.Fatal(err)
	}
	waitDispatch(t, done)
	for _, agent := range []api.CreateAgentInput{
		{ID: workerID, ProjectID: projectID, Name: "worker", Role: "worker", Provider: "codex", ToolBudgetLimit: 1},
		{ID: overseerID, ProjectID: projectID, Name: "overseer", Role: "orchestrator", Provider: "codex", ToolBudgetLimit: 1},
		{ID: archivedID, ProjectID: projectID, Name: "archived", Role: "worker", Provider: "codex", ToolBudgetLimit: 1},
	} {
		done = fixture.serve(t)
		if _, err := client.CreateAgent(ctx, agent); err != nil {
			t.Fatal(err)
		}
		waitDispatch(t, done)
	}
	account, err := kernel.AccountIDFromBytes(mustIDBytes(t, accountID))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := fixture.store.LinkAccount(ctx, kernel.NewAccount{ID: account, Provider: kernel.ProviderCodex, Home: filepath.Join(t.TempDir(), ".codex"), Label: "test"}, mustKernelTime(t, 1050)); err != nil {
		t.Fatal(err)
	}
	archived, found, err := fixture.store.Agent(ctx, mustAgentID(t, archivedID))
	if err != nil || !found {
		t.Fatalf("archived agent = %+v, found=%v, err=%v", archived, found, err)
	}
	archive := true
	if _, err := fixture.store.UpdateAgent(ctx, archived.ID, archived.Revision, kernel.AgentPatch{Archived: &archive}, mustKernelTime(t, 1100)); err != nil {
		t.Fatal(err)
	}

	selectAccount := func(agentID string, expected uint64) error {
		done := fixture.serve(t)
		_, err := client.SelectAgentAccount(ctx, api.AgentAccountSelectInput{AgentID: agentID, ExpectedRevision: expected, AccountID: accountID})
		waitDispatch(t, done)
		return err
	}
	if err := selectAccount(workerID, 1); err != nil {
		t.Fatalf("worker account selection = %v", err)
	}
	if worker, found, err := fixture.store.Agent(ctx, mustAgentID(t, workerID)); err != nil || !found || worker.AccountID.String() != accountID || worker.Revision.Int64() != 2 {
		t.Fatalf("selected worker account = %+v, found=%v, err=%v", worker, found, err)
	}
	for _, test := range []struct {
		name     string
		agentID  string
		...
		{name: "stale revision", agentID: workerID, expected: 1, code: api.RemoteRevisionConflict},
		{name: "overseer", agentID: overseerID, expected: 1, code: api.RemoteConflict},
		{name: "archived worker", agentID: archivedID, expected: 1, code: api.RemoteConflict},
		{name: "missing agent", agentID: testID(65), expected: 1, code: api.RemoteNotFound},
	} {
		t.Run(test.name, func(t *testing.T) {
			err := selectAccount(test.agentID, test.expected)
			var remote *api.RemoteError
