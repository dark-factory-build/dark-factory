...
func (daemon *Daemon) exactIntakeIssue(ctx context.Context, source kernel.IntakeSource, number uint64) (maintainer.Issue, error) {
	...
	page, err := daemon.github.Issues(ctx, source.GitHubRepositoryName, source.GitHubRepositoryID, 1, "", number)
	if err != nil {
...
func (daemon *Daemon) previewIntake(ctx context.Context, source kernel.IntakeSource, page uint32, tick bool) api.IntakeResult {
	...
	observed, err := daemon.github.Issues(ctx, source.GitHubRepositoryName, source.GitHubRepositoryID, page, source.LabelFilter, 0)
	if err != nil {
		return intakeFailure(err)
	}
