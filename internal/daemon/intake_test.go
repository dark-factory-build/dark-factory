...
func TestIntakeAcceptedContentSurvivesLargeBacklogAndMetadataChanges(t *testing.T) {
	for _, linear := range []bool{false, true} {
		t.Run(fmt.Sprint("linear=", linear), func(t *testing.T) {
			...
			spec := kernel.NewIntakeSource{ID: id, ProjectID: project, TargetRepositoryID: kernel.RepositoryID(project), OverseerAgentID: agent, GitHubRepositoryID: 42, GitHubRepositoryName: "team/issues", Policy: kernel.IntakePolicyManual, PollSeconds: 60, AdmissionLimit: 1}
			if linear {
				spec.GitHubRepositoryID = 0
				spec.LinearTeamID = "11111111-1111-4111-8111-111111111111"
				spec.GitHubRepositoryName = "Engineering"
			}
			source, err := fixture.store.CreateIntakeSource(ctx, spec, mustKernelTime(t, 103))
			if err != nil {
				t.Fatal(err)
			}
			...
			if linear {
				issue.NodeID = "22222222-2222-4222-8222-222222222222"
				issue.URL = "https://linear.app/acme/issue/ENG-99999/test"
			}
			...
			fixture.daemon.intakeIssues = func(_ context.Context, repository string, repositoryID uint64, page uint32, label string, number uint64) (maintainer.IssuePage, error) {
				if repository != spec.GitHubRepositoryName || repositoryID != spec.GitHubRepositoryID {
					t.Fatal("wrong source")
				}
				if number != 0 {
					exactReads++
					return maintainer.IssuePage{RepositoryID: spec.GitHubRepositoryID, Issues: []maintainer.Issue{issue}}, nil
				}
				if discover {
					return maintainer.IssuePage{RepositoryID: spec.GitHubRepositoryID, Issues: []maintainer.Issue{issue}}, nil
				}
				...
				return maintainer.IssuePage{RepositoryID: spec.GitHubRepositoryID, Issues: []maintainer.Issue{}, NextPage: &next}, nil
			}
			...
			discover = true
			if got := fixture.daemon.Intake(ctx, api.IntakeInput{Action: "preview", SourceID: id.String(), Page: 1}); got.State != "ok" || got.SourceID != id.String() || len(got.Candidates) != 1 {
