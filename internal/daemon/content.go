...
func contentDTO(v kernel.ContentRevision) api.Content {
	return api.Content{ID: v.ID.String(), ProjectID: v.ProjectID.String(), Kind: string(v.Kind), Title: v.Title, Description: v.Description, Author: v.Author, SourceReferences: v.SourceReferences, Revision: uint64(v.Revision.Int64()), LatestRevision: uint64(v.LatestRevision.Int64()), Deprecated: v.Deprecated}
}
...
func (daemon *Daemon) content(ctx context.Context, call api.Call) api.Reply {
	...
	makeSpec := func() (kernel.NewContent, error) {
		...
		return kernel.NewContent{ID: id, ProjectID: p, Kind: kernel.ContentKind(input.Kind), Title: input.Title, Description: input.Description, Body: input.Body, Author: author, SourceReferences: input.SourceReferences}, nil
	}
	switch call.Kind() {
