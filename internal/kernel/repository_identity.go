...
func validateRepositoryBindings(ctx context.Context, connection *sql.Conn) error {
 ...
 EXISTS(SELECT 1 FROM project_content_revisions c LEFT JOIN content_repository_bindings b ON b.content_id = c.id AND b.content_revision = c.revision LEFT JOIN project_repositories r ON r.id = b.repository_id WHERE c.commit_oid IS NOT NULL AND (r.id IS NULL OR r.project_id <> c.project_id)) OR
