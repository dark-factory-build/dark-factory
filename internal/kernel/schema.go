...
var schemaStatements = []string{
    ...
    resolution_kind TEXT CHECK (resolution_kind IS NULL OR resolution_kind IN ('reply', 'stale', 'action_cancel_run')),
    ...
    CHECK (status = 'open' AND delivery_id IS NULL OR status IN ('delivering', 'delivery_unknown') AND delivery_id IS NOT NULL OR status = 'resolved' AND (resolution_kind = 'reply' AND delivery_id IS NOT NULL OR resolution_kind = 'action_cancel_run' AND delivery_id IS NULL) OR status = 'stale'),
    CHECK ((status IN ('resolved', 'stale')) = (closed_at_ms IS NOT NULL)),
    CHECK (status = 'resolved' AND resolution_kind IN ('reply', 'action_cancel_run') OR status = 'stale' AND resolution_kind = 'stale' OR status IN ('open', 'delivering', 'delivery_unknown') AND resolution_kind IS NULL),
