...
func newLegacyDatabase(t *testing.T, persistWAL bool, version int, extra ...string) (string, map[string][]string) {
	...
	downgrade := []string{"DROP TABLE content_repository_bindings", "DROP TABLE task_repository_bindings", "DROP TABLE project_repositories", "DROP TABLE continuations", fmt.Sprintf("PRAGMA user_version = %d", version), "COMMIT"}
...
func TestSchemaDigestsArePinned(t *testing.T) {
	for _, pin := range []struct {
		...
		{"current", schemaStatements, "3dd09f64e28fcb94ba6999129defe92286a10907e8ae5b6e4be4b37fb3281478"},
