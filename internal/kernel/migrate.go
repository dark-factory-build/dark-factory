...
func v32SchemaStatements() []string {
	var statements []string
	for _, statement := range schemaStatements {
		_, name := schemaObjectIdentity(statement)
		if name != "production_records" && name != "publication_tasks" {
			statements = append(statements, statement)
		}
	}
	return statements
...
func v31SchemaStatements() []string {
	var statements []string
	for _, statement := range v32SchemaStatements() {
		_, name := schemaObjectIdentity(statement)
...
func migratableSchema(version int) ([]string, bool) {
	...
	case v32UserVersion:
		return v32SchemaStatements(), true
	case v31UserVersion:
...
func migrateV31Transaction(ctx context.Context, connection *sql.Conn) error {
	...
	target := expectedSchemaOf(v32SchemaStatements())
	for _, name := range []string{"mission_task_bindings", "mission_task_bindings_mission"} {
		if _, err := connection.ExecContext(ctx, target[name].sql); err != nil {
			return err
		}
	}
	if _, err := connection.ExecContext(ctx, fmt.Sprintf("PRAGMA user_version = %d", v32UserVersion)); err != nil {
		return err
	}
	return validateSchemaVersion(ctx, connection, v32UserVersion, v32SchemaStatements())
}

func migrateV32Transaction(ctx context.Context, connection *sql.Conn) error {
	if err := validateSchemaVersion(ctx, connection, v32UserVersion, v32SchemaStatements()); err != nil {
		return err
	}
	target := expectedSchemaOf(schemaStatements)
	for _, name := range []string{"production_records", "publication_tasks"} {
		if _, err := connection.ExecContext(ctx, target[name].sql); err != nil {
			return err
		}
	}
