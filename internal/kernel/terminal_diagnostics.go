...
func (store *Store) SaveTerminalDiagnostics(ctx context.Context, value TerminalDiagnostics) error {
	if store == nil || ctx == nil || value.RunID.zero() || value.Floor > value.Head || uint64(len(value.Payload)) > uint64(MaxTerminalDiagnosticsBytes) || uint64(len(value.Payload)) > value.Head-value.Floor {
