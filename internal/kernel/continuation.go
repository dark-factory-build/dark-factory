...
func (store *Store) ResolveHumanContinuationForAttempt(ctx context.Context, digest AttemptDigest, requestID HumanRequestID, expected Revision, reply string, at UnixMillis) (bool, error) {
	...
	updated, err := tx.connection.ExecContext(ctx, `UPDATE human_requests SET status='resolved', resolution_kind='reply', closed_at_ms=?, revision=revision+1, updated_at_ms=? WHERE id=? AND status='open' AND revision=?`, at.Int64(), at.Int64(), request.ID.Bytes(), expected.Int64())
	if err := requireOneRow(updated, err); err != nil {
		return false, tx.Rollback(err)
	}
	...
	} else if _, err := promoteContinuationOnConnection(ctx, tx.connection, continuation.ID, at); err != nil {
		return false, tx.Rollback(err)
	}
	if continuation.State == ContinuationWaiting {
		if _, err := promoteContinuationOnConnection(ctx, tx.connection, continuation.ID, at); err != nil {
			return false, tx.Rollback(err)
		}
	}
	if err := tx.Commit(ctx); err != nil {
...
func promoteContinuationOnConnection(ctx context.Context, connection *sql.Conn, id ContinuationID, at UnixMillis) (*Task, error) {
	...
	updated, err = connection.ExecContext(ctx, `UPDATE continuations SET state='resolved', revision=revision+1, updated_at_ms=? WHERE id=? AND state='queued'`, at.Int64(), id.Bytes())
	if err := requireOneRow(updated, err); err != nil {
		return nil, err
	}
