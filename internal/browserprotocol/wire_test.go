...
func decodeFixtureControl(name string, data []byte) (ControlFrame, error) {
	switch name {
	case "task_attachment", "pair_prove", "auth_prove", "state_get", "state_watch", "human_request_detail_get", "task_enqueue", "project_create", "repositories_get", "repository_mutate", "terminal_target_get":
...
func TestManifestMatchesImplementedRegistry(t *testing.T) {
	...
	if manifest.Name != "dark-factory/browser" || len(manifest.Control) != 81 || len(manifest.Terminal.Opcodes) != 2 {
