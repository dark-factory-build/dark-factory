...
func decodeFixtureControl(name string, data []byte) (ControlFrame, error) {
	switch name {
	case "attachment_retention", "task_attachment", "pair_prove", "auth_prove", "state_get", "state_watch", "human_request_detail_get", "task_enqueue", "project_create", "repositories_get", "repository_mutate", "terminal_target_get":
...
func encodeDecoded(frame ControlFrame) ([]byte, error) {
	...
	case TaskAttachmentChunk:
		return EncodeTaskAttachment(frame.ID, value)
	case TaskAttachmentResult:
		return EncodeTaskAttachmentResult(frame.ID, value)
	case TaskEnqueue:
	...
	case AttachmentRetention:
		return encodeControl(TypeAttachmentRetention, frame.ID, value)
	case AttachmentRetentionResult:
		return EncodeAttachmentRetentionResult(frame.ID, value)
	case ProjectLimits:
	...
	case ProjectCreate:
		return encodeControl(TypeProjectCreate, frame.ID, value)
	case ProjectCreateResult:
		return EncodeProjectCreateResult(frame.ID, value)
	case RepositoriesGet:
		return encodeControl(TypeRepositoriesGet, frame.ID, value)
	case Repositories:
		return EncodeRepositories(frame.ID, value)
	case RepositoryMutate:
		return encodeControl(TypeRepositoryMutate, frame.ID, value)
	case RepositoryMutateResult:
		return EncodeRepositoryMutateResult(frame.ID, value)
	case Intake:
		return encodeControl(TypeIntake, frame.ID, value)
	case IntakeResult:
		return EncodeIntakeResult(frame.ID, value)
...
func TestManifestMatchesImplementedRegistry(t *testing.T) {
	...
	if manifest.Name != "dark-factory/browser" || len(manifest.Control) != 83 || len(manifest.Terminal.Opcodes) != 2 {
...
func TestIntakeEmptyListsEncodeAsSharedClientFixture(t *testing.T) {
	want := fixtureBytes(t, "intake_result.json")
	frame, err := DecodeServerControl(want)
	if err != nil {
		t.Fatal(err)
	}
	value := frame.Body.(IntakeResult)
	value.Sources[0].TrustedAuthors = nil
	value.Candidates[0].Labels = nil
	got, err := EncodeIntakeResult(frame.ID, value)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(got, want) {
		t.Fatalf("empty required lists changed shared wire fixture: %s", got)
	}
}

func TestIntakeSourceConfigurationRequiresExactWireFields(t *testing.T) {
	for _, field := range []string{"repository", "trusted_authors"} {
		for _, rename := range []bool{false, true} {
			var frame map[string]any
			if err := json.Unmarshal(fixtureBytes(t, "intake_result.json"), &frame); err != nil {
				t.Fatal(err)
			}
			source := frame["body"].(map[string]any)["sources"].([]any)[0].(map[string]any)
			if rename {
				source[strings.ToUpper(field)] = source[field]
			}
			delete(source, field)
			...
			if _, err := DecodeServerControl(wire); !errors.Is(err, ErrMalformed) {
				t.Fatalf("missing/case-changed source field %s accepted: %v", field, err)
			}
		}
	}
}
