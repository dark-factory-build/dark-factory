...
func TestAgentItemServesExactlyThePublicFields(t *testing.T) {
	...
	want := []string{"id", "project_id", "name", "role", "provider", "appearance", "paused", "archived", "model", "reasoning_effort", "effective_model", "effective_reasoning_effort", "model_source", "revision", "account_id", "idle_policy", "idle_after_seconds", "idle_instruction", "idle_run_budget", "idle_runs_used"}
...
func TestHumanRequestPublicPrivacyAndDetailBounds(t *testing.T) {
	...
	yielded.CancelRun = &HumanRequestCancelRunDescriptor{ExpectedRequestRevision: yielded.Revision}
	if _, err := EncodeHumanRequestDetail("detail", yielded); err == nil {
		t.Fatal("yielded detail without a run revision accepted")
	}
}
