...
func decodeCall(domain byte, bearer credential, encoded []byte) (Call, RemoteErrorCode) {
	...
	case CallContentCreate, CallContentRevise:
		if err := decodeExact(request.Params, &call.content); err != nil || !validText(call.content.ID, 1, 64) || !validText(call.content.ProjectID, 1, 64) || !validText(call.content.Author, 1, 256) || !validText(call.content.Title, 1, 1024) || !validText(call.content.Description, 0, 4096) || !validText(call.content.Body, 0, 1<<20) || !validText(call.content.SourceReferences, 0, 32768) || (call.content.Kind != "procedure" && call.content.Kind != "acceptance_scenario") {
			return Call{}, RemoteInvalidRequest
		}
	...
	case CallContentEvidence:
		if err := decodeExact(request.Params, &call.contentEvidence); err != nil || !validText(call.contentEvidence.ID, 1, 64) || !validText(call.contentEvidence.ProjectID, 1, 64) || !validText(call.contentEvidence.ContentID, 1, 64) || call.contentEvidence.ContentRevision == 0 || !validText(call.contentEvidence.TestedSource, 1, 4096) || !validText(call.contentEvidence.Environment, 0, 4096) || !validText(call.contentEvidence.Location, 0, 4096) || !validText(call.contentEvidence.Evaluator, 1, 256) || !validText(call.contentEvidence.Judgment, 0, 8192) || (call.contentEvidence.Result != "passed" && call.contentEvidence.Result != "failed" && call.contentEvidence.Result != "incomplete" && call.contentEvidence.Result != "not_run") {
			return Call{}, RemoteInvalidRequest
		}
...
func replyMatches(kind CallKind, reply replyKind) bool {
	...
	case CallContentCreate, CallContentRevise, CallContentDeprecate, CallContentList, CallContentRead, CallContentBody, CallContentEvidence, CallContentAttach:
